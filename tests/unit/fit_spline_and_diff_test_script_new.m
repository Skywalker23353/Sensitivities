...
diffDir = fullfile(scriptDir, '..', '..', 'data', 'processed', 'diff_figs_n');
...
y1 = 2*(x.^5) - 5*(x.^2) + 1 ;%+ 0.1*randn(size(x));
y1_interp = 2*(x_interp.^5) - 5*(x_interp.^2) + 1 ;%+ 0.1*randn(size(x_interp));
y1_interp_prime = 10.*(x_interp.^4) - 10*(x_interp);%+ 0.1*randn(size(x_interp));

fs = 33; % Font size for plots
%% Plot original data
h = figure('WindowState','maximized');
plot(x, y1, '-ob', 'DisplayName', 'y : 2x^5 - 5x^2 + 1','MarkerSize',15);
hold on;
plot(x_interp, y1_interp, '-ok', 'DisplayName', 'y (Interp) : 2x^5 - 5x^2 + 1','MarkerSize',15);
hold on
plot(x_interp, y1_interp_prime, '-+r', 'DisplayName', 'y''(Interp) : 10x^4 -10x','MarkerSize',15);
xlabel('x');
% ylabel('y');
...
h = figure('WindowState','maximized');
subplot(2,1,1);
plot(x_interp, y1_interp, '-ok', 'DisplayName', 'y : 2x^5 - 5x^2 + 1','MarkerSize',15);
hold on;
...
h = figure('WindowState','maximized');
subplot(2,1,1);
plot(x_interp, y1_interp_prime, '-ok', 'DisplayName', 'y'' : 10x^4 - 10x','MarkerSize',15);
hold on;
