...
h5filename = 'Reactants_42';
% Load paths and constants
sensitivity_constants_delc_0_1_chem_src_model;  
% Load input field configurations
C_cond_field_name_chem_src_model; 
% Load LES data
...
[Spline_fields] = set_latex_labels(Spline_fields,field_configs);
%%
[Spline_fields,LES] = compute_function_using_beta_pdf(Spline_fields,"Comb",LES,'type','dfdc_rz','epsilon',1e-3);
[Spline_fields,LES] = compute_function_using_beta_pdf(Spline_fields,"Noz",LES,'type','dfdc_rz','epsilon',1e-3);
%% Compute sensitivities
% Populate chem source sensitivities structure
Sensitivities = struct();
fieldNames = fieldnames(Spline_fields);
nfields = size(fieldNames,1);
for i = 1:nfields
    Sensitivities.Comb.(fieldNames{i}) = Spline_fields.(fieldNames{i}).Comb.dfdc_rz;
    Sensitivities.Noz.(fieldNames{i}) = Spline_fields.(fieldNames{i}).Noz.dfdc_rz;
end
%Constructing heat release sensitivities
[Sensitivities] = construct_hrr_sensitivities(Sensitivities,Constant);
%% Normalize fields
[Sensitivities] = normalize_sensitivities(Sensitivities,Constant);
%% Plot dfdr
if plot_fields
...
%% Add c_field to sensitivities structure
